...
func (tc *TrayCache) DoCache() error {
	if tc.IsCached() {
		return nil
	}
	...
	assetTmpFile, err := tc.extract(tmpDir)
	if err != nil {
		return err
	}
...
func (tc *TrayCache) EnsureIsCached() error {
	if !tc.IsCached() {
		err := tc.DoCache()
		if err != nil {
			return err
...
func NewWindowsTrayCache(destDir string) *TrayCache {
	return &TrayCache{BinaryCache{binaryName: constants.TrayBinaryName, archiveURL: constants.GetCRCWindowsTrayDownloadURL(), destDir: destDir}}
}
