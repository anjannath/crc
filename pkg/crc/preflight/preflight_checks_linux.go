...
func fixKvmEnabled() (bool, error) {
	cmd := exec.Command("sudo", "modprobe", "kvm")
	err := cmd.Run()
	if err != nil {
		return false, err
	}
...
func fixLibvirtInstalled() (bool, error) {
	cmd := exec.Command("sudo", "yum", "install", "-y", "libvirt", "libvirt-daemon-kvm", "qemu-kvm")
	err := cmd.Run()
	if err != nil {
		return false, errors.New("Could not install required packages")
	}
	return true, nil
}
...
func fixLibvirtEnabled() (bool, error) {
	...
	cmd := exec.Command("sudo", path, "enable", "libvirtd")
	err = cmd.Run()
	if err != nil {
		return false, err
	}
...
func fixUserPartOfLibvirtGroup() (bool, error) {
	...
	cmd := exec.Command("sudo", "usermod", "-a", "-G", "libvirt", currentUser.Username)
	err = cmd.Run()
	if err != nil {
		return false, err
	}
...
func fixLibvirtServiceRunning() (bool, error) {
	...
	cmd := exec.Command("sudo", path, "start", "libvirtd")
	err = cmd.Run()
	if err != nil {
		return false, err
	}
...
func fixDockerMachineDriverInstalled() (bool, error) {
	...
	out, err := os.Create(kvmDriverBinaryPath)
	if err != nil {
		return false, err
	}
	defer out.Close()
