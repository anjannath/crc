...
func CreateHost(machineConfig config.MachineConfig) *hyperv.Driver {
	...
	for _, dir := range machineConfig.SharedDirs {
		...
		hypervDriver.SharedDirs = append(hypervDriver.SharedDirs, sharedDir)
	}
	return hypervDriver
}
...
func smbShareUNCPathFromTag(tag string) string {
	// get ip address from one of the connected interfaces
	// use that to mount the share
	hostname, _, err := powershell.Execute("hostname")
	if err == nil {
